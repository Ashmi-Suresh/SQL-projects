...
-- Answer : Here we see four columns of the movie table contains null values. 
...
ORDER BY 
 ...
 /* Answer :March is the month where highest number of movies is usually produced.
As per observation we understood the month-wise trend of movies, we can look at the other details like who produces more movies is it India or USA. */
...
-- Answer : India &  USA produced more than 1000 movies in the year 2019.
...
-- Answer: RSVP Movies is planning to make a movie from one of the distinct genres.
...
-- Answer  : Drama as a genre is extremely used & is therefore has the highest number of movies produced.
...
-- Answer : More than 3000 movies belong to only one genre.
...
-- Answer : Apparently the genre 'Drama' has the average duration of 106.77 mins.
...
-- Answer : Thriller movies is in top 3 among all genres in terms of number of movies
...
/* Answer: The minimum and maximum values are in the expected range in each column of the ratings table. 
Hence no outliers are observed in the table.*/
...
-- Answer : We do see our favourite movie in the top movies with an avg rating of 9.6
...
-- Answer : 7 is the highest number for movies with median rating.
...
/* Answer : We can use both if RANK() or DENSE_RANK() , hence the answer would be 
...
/* Answer : We can see 'Drama' genre has highest movie count during March 2017
		in the USA had more than 1,000 votes with total 16 movies */
...
/* Answer : There are 8 movies which begin with “The” in the title
...
-- Answer : Of the movies released between 1 April 2018 and 1 April 2019, 361 Movies got a median rating of 8
...
-- Answer : As you can see there are no Null value in the column 'name'.
...
/* Answer : The top three directors in the top three genres are,
         1. James Mangold.
         2. Soubin Shahir.
         3. Joe Russo. */
...
-- Answer : The Top 3 Production house are 1. Marvel Studios 2. Twentieth Century Fox 3. Warner Bros.
...
-- Answer : Based on their average ratings 'Vijay Sethupathi' is Top actor in india.
...
-- Answer : Based on their average ratings 'Tapsee Pannu' is Top actoress in india.
...
/* Answer : The names of the top two production houses that have produced the highest number of hits among multilingual movies are,
			1. Star Cinema.
...
SELECT 
	name, SUM(total_votes) AS total_votes,
...
/* Answer : The top 3 actresses based on number of Super Hit movies with average rating >8 in drama genre are,
		 1. Sangeetha Bhat
         2. Fatmire Sahiti
         3. Adriana Matoshi */
...
-- Answer : As we can see "A.L Vijay" and "Andrew Jones" has the same number of movies and "A.L Vijay" Tops the list.
